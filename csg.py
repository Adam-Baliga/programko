...
class CSG_object_node:
    ...
    def rotate(self,inverse_rot_matrix): # the input should be the inverse rotation matrix
        if self.is_leaf():
            self.rotation = self.rotation @ inverse_rot_matrix
        else:
            self.left.rotate(inverse_rot_matrix)
            self.right.rotate(inverse_rot_matrix)
